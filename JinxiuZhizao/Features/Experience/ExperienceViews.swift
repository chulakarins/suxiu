...
private extension Color {
    ...
    // “六条文化线索”局部使用的暖纸色谱，不影响其他页面与组件。
    static let cultureCluePaper = Color(red: 0.969, green: 0.953, blue: 0.918)       // #F7F3EA
    static let cultureClueCard = Color(red: 0.988, green: 0.980, blue: 0.961)        // #FCFAF5
    static let cultureClueInk = Color(red: 0.110, green: 0.106, blue: 0.094)         // #1C1B18
    static let cultureClueSecondary = Color(red: 0.439, green: 0.420, blue: 0.388)   // #706B63
    static let cultureClueIcon = Color(red: 0.251, green: 0.239, blue: 0.220)        // #403D38
    static let cultureClueIconSurface = Color(red: 0.925, green: 0.902, blue: 0.863) // #ECE6DC
    static let cultureClueDivider = Color(red: 0.867, green: 0.839, blue: 0.796)     // #DDD6CB
    static let cultureClueCinnabar = Color(red: 0.639, green: 0.302, blue: 0.247)    // #A34D3F
}
...
struct HeritageHomeView: View {
    ...
    private var topicGrid: some View {
        VStack(alignment: .leading, spacing: 18) {
            cultureClueHeader
            featuredCultureClue(topics[1])

            HStack(alignment: .top, spacing: 12) {
                ForEach([topics[2], topics[3]]) { topic in
                    mediumCultureClue(topic)
                        .frame(maxWidth: .infinity)
                }
            }

            compactCultureClues([topics[0], topics[4], topics[5]])
        }
        .padding(18)
        .background(Color.cultureCluePaper, in: RoundedRectangle(cornerRadius: 24, style: .continuous))
    }

    private var cultureClueHeader: some View {
        VStack(alignment: .leading, spacing: 7) {
            HStack(spacing: 8) {
                Rectangle()
                    .fill(Color.cultureClueCinnabar)
                    .frame(width: 24, height: 1)
                Text("EXPLORE")
                    .font(.system(size: 10, weight: .semibold))
                    .tracking(1.8)
                    .foregroundStyle(Color.cultureClueCinnabar)
            }

            Text("六条文化线索")
                .font(.system(size: 23, weight: .semibold))
                .tracking(-0.45)
                .foregroundStyle(Color.cultureClueInk)
        }
    }

    private func featuredCultureClue(_ topic: HeritageTopic) -> some View {
        Button(action: { selectedTopic = topic }) {
            VStack(alignment: .leading, spacing: 0) {
                SuxiuAspectFillImage(name: topic.image, height: 184)

                HStack(alignment: .top, spacing: 12) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .fill(Color.cultureClueIconSurface)
                        cultureClueSymbol(topic.icon, size: 17)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        Text(topic.subtitle)
                            .font(.system(size: 12))
                            .foregroundStyle(Color.cultureClueSecondary)
                    }
            ...
            .background(Color.cultureClueCard)
            .clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
            .overlay {
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(Color.cultureClueDivider.opacity(0.62), lineWidth: 0.7)
            }
        }
        .buttonStyle(SuxiuPressStyle())
        .accessibilityLabel("\(topic.title)，\(topic.subtitle)，收录 \(topic.count) 项")
    }

    private func mediumCultureClue(_ topic: HeritageTopic) -> some View {
        Button(action: { selectedTopic = topic }) {
            VStack(alignment: .leading, spacing: 10) {
                SuxiuAspectFillImage(name: topic.image, height: 104, cornerRadius: 10)

                HStack(spacing: 7) {
                    cultureClueSymbol(topic.icon, size: 14)
                    Text(topic.title)
            ...
            .padding(12)
            .background(Color.cultureClueCard, in: RoundedRectangle(cornerRadius: 16, style: .continuous))
        }
        .buttonStyle(SuxiuPressStyle())
        .accessibilityLabel("\(topic.title)，\(topic.subtitle)，收录 \(topic.count) 项")
    }

    private func compactCultureClues(_ compactTopics: [HeritageTopic]) -> some View {
        VStack(spacing: 0) {
            ForEach(Array(compactTopics.enumerated()), id: \.element.id) { index, topic in
                Button(action: { selectedTopic = topic }) {
                    HStack(spacing: 12) {
                        cultureClueSymbol(topic.icon, size: 17)
                            .frame(width: 26)
                ...
                .buttonStyle(SuxiuPressStyle(pressedScale: 0.99))
                .accessibilityLabel("\(topic.title)，\(topic.subtitle)，收录 \(topic.count) 项")

                if index < compactTopics.count - 1 {
                    Rectangle()
                        .fill(Color.cultureClueDivider)
                        .frame(height: 0.5)
                        .padding(.leading, 38)
                }
            }
    ...
    private func cultureClueSymbol(_ systemName: String, size: CGFloat) -> some View {
        Image(systemName: systemName)
            .symbolRenderingMode(.monochrome)
            .font(.system(size: size, weight: .medium))
            .foregroundStyle(Color.cultureClueIcon)
    }

    private var dailyArchive: some View {
        VStack(alignment: .leading, spacing: 16) {
            SuxiuSectionHeader(eyebrow: "DIGITAL ARCHIVE", title: "数字档案")
            HStack(spacing: 14) {
...
struct RestorationDemoView: View {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        Button(action: toggleComparison) {
                            Label(
                                showingRestored ? "返回原始影像" : "展示完整修复",
                                systemImage: showingRestored ? "arrow.uturn.backward" : "wand.and.stars"
                            )
                                .font(.system(size: 15, weight: .semibold))
                                .foregroundStyle(.white)
                                .frame(maxWidth: .infinity)
    ...
    private var showingRestored: Bool {
        comparisonProgress >= 0.96
    }

    private var comparisonViewer: some View {
        GeometryReader { proxy in
            let width = proxy.size.width
            let dividerX = min(max(width * comparisonProgress, 1), width - 1)
            let handleX = min(max(width * comparisonProgress, 19), width - 19)

            ZStack(alignment: .leading) {
                SuxiuAspectFillImage(name: "OpenSuxiuBogu", height: 330)
                    .saturation(0.32)
                    .contrast(0.82)
                    .overlay(Color.brown.opacity(0.22))
                ...
                SuxiuAspectFillImage(name: "OpenSuxiuBogu", height: 330)
                    ...
                    .mask {
                        Rectangle()
                            .frame(width: width * comparisonProgress)
                            .frame(maxWidth: .infinity, alignment: .leading)
                    }
                ...
                VStack {
                    HStack {
                        SuxiuTag(text: "AI 推测预览", tint: .heritageJade)
                        Spacer()
                        SuxiuTag(text: "原始影像", tint: .orange)
                    }
                    Spacer()
                }
                .padding(14)
            ...
            .gesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        comparisonProgress = min(max(value.location.x / max(width, 1), 0), 1)
                    }
            )
        }
        .frame(height: 330)
        ...
        .accessibilityLabel("原始影像与 AI 推测预览对比")
        .accessibilityValue("显示 \(Int(comparisonProgress * 100))% AI 推测预览")
        .accessibilityAdjustableAction { direction in
            let delta: CGFloat = direction == .increment ? 0.1 : -0.1
            withAnimation(SuxiuMotion.compare(reduceMotion: reduceMotion)) {
                comparisonProgress = min(max(comparisonProgress + delta, 0), 1)
            }
        }
    }
    ...
    private func toggleComparison() {
        withAnimation(SuxiuMotion.compare(reduceMotion: reduceMotion)) {
            comparisonProgress = showingRestored ? 0 : 1
        }
    }
...
struct CourseDetailView: View {
    ...
    @State private var completionFeedbackCount = 0
    ...
    var body: some View {
        ...
        .sensoryFeedback(.success, trigger: completionFeedbackCount)
    }
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 8) {
                ForEach(Array(course.steps.indices), id: \.self) { index in
                    Button(action: { selectStep(index) }) {
                        Text("第 \(index + 1) 步")
                            .font(.system(size: 11, weight: .semibold))
            ...
            .id(currentStep)
            .transition(
                SuxiuMotion.directionalTransition(
                    forward: stepMovesForward,
    ...
    private func advance() {
        if currentStep < course.steps.count - 1 {
    ...
    private func selectStep(_ index: Int) {
        guard course.steps.indices.contains(index), index != currentStep else { return }
        stepMovesForward = index > currentStep
...
struct CommunityHubView: View {
    ...
    var body: some View {
        ...
        .sensoryFeedback(.selection, trigger: selectedFeed)
        .sensoryFeedback(.impact(weight: .light), trigger: joinedChallenge)
    }
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 9) {
                ForEach(feedFilters, id: \.self) { filter in
                    Button(action: { selectFeed(filter) }) {
                        Text(filter)
                            .font(.system(size: 12, weight: .semibold))
                            ...
                            .background {
                                ZStack {
                                    ...
                                    if selectedFeed == filter {
                                        Capsule()
                                            .fill(Color.heritageBlue)
                                            .matchedGeometryEffect(id: "community-feed-selection", in: feedSelectionNamespace)
                                    }
                                }
                            }
                    }
                    .buttonStyle(SuxiuPressStyle(pressedScale: 0.95))
                }
            }
        }
    }
    ...
    private var processJournal: some View {
                    ...
                    HStack {
                        Button(action: { toggleLike(entry.id) }) {
                            Label("\(entry.likes + (likedEntries.contains(entry.id) ? 1 : 0))", systemImage: likedEntries.contains(entry.id) ? "heart.fill" : "heart")
                                .contentTransition(SuxiuMotion.numericTransition(reduceMotion: reduceMotion))
                        }
                        .buttonStyle(SuxiuPressStyle(pressedScale: 0.90))
        ...
        .animation(SuxiuMotion.content(reduceMotion: reduceMotion), value: selectedFeed)
        .sensoryFeedback(.impact(weight: .light), trigger: likedEntries)
    }
    ...
    private func selectFeed(_ filter: String) {
        guard selectedFeed != filter else { return }
        withAnimation(reduceMotion ? nil : SuxiuMotion.content(reduceMotion: false)) {
            selectedFeed = filter
        }
    }
...
private struct CommunityDiscussionView: View {
    ...
    @State private var sentFeedbackCount = 0
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                ScrollViewReader { proxy in
                    ...
                    .onChange(of: comments.count) { _, newCount in
                        ...
                        withAnimation(SuxiuMotion.content(reduceMotion: reduceMotion)) {
                            proxy.scrollTo("comment-\(newCount - 1)", anchor: .bottom)
                        }
                    }
                }
            ...
            .sensoryFeedback(.success, trigger: sentFeedbackCount)
...
struct MarketplaceHubView: View {
    ...
    private var modePicker: some View {
        HStack(spacing: 5) {
            ForEach(modes.indices, id: \.self) { index in
                Button(action: { selectMode(index) }) {
                    Text(modes[index])
                        ...
                        .background {
                            if selectedMode == index {
                                RoundedRectangle(cornerRadius: 11)
                                    .fill(Color.heritageBlue)
                                    .matchedGeometryEffect(id: "market-mode-selection", in: modeSelectionNamespace)
                            }
                        }
                }
                .buttonStyle(SuxiuPressStyle(pressedScale: 0.96))
    ...
    private func selectMode(_ index: Int) {
        guard modes.indices.contains(index), index != selectedMode else { return }
        withAnimation(reduceMotion ? nil : SuxiuMotion.content(reduceMotion: false)) {
            selectedMode = index
        }
    }
