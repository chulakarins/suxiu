...
struct ProfileView: View {
    ...
    private var menuSection: some View {
        VStack(spacing: 0) {
            menuRowContent(icon: "bag", title: "我的订单", trailingText: "比赛演示")
            menuRowContent(icon: "heart", title: "我的收藏", trailingText: "比赛演示")
            Button(action: { showingGenerationHistory = true }) {
                menuRowContent(icon: "clock", title: "生成历史", showsChevron: true)
            }
...
struct HomeView: View {
    ...
    private func tabLayer<Content: View>(
        ...
        let restingOffset: CGFloat = tag < selectedTab ? -10 : 10

        return content()
            .opacity(isSelected ? 1 : 0)
            .offset(x: isSelected || reduceMotion ? 0 : restingOffset)
        ...
        Button(action: {
            ...
            VStack(spacing: 3) {
                Image(systemName: selectedTab == tag ? selectedIcon : icon)
                    ...
                    .scaleEffect(selectedTab == tag && !reduceMotion ? 1.04 : 1.0)
